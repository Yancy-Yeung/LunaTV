...
- ❤️ **收藏 + 继续观看**：支持 Kvrocks/Redis/Upstash 存储，多端同步进度。Redis支持选择数据库（0-15）。
...
REDIS_DATABASE=1  # 可选，Redis数据库编号，默认为0
```
...
| REDIS_PASSWORD                      | redis 连接密码（可选）     | 密码字符串               | 空                                                                                                                         |
| REDIS_DATABASE                      | redis 数据库编号（可选）   | 0-15                     | 0                                                                                                                          |
